 ...
 */
public class AggregationDistribution extends BasicNode implements Generateable {

  List<String> aggregate = new ArrayList<>();
  List<String> distribute = new ArrayList<>();

  public AggregationDistribution() {
    nodeType = "AggregationDistribution";
  }
  ...
  public void createQuery() {
    ...
    for (String target : aggregate) {
      attributes = new HashMap<>();
      attributes.put("key", target);
      Neo4j.createNode(null, attributes);
      Neo4j.addRelation(getKey(), target, "has", null);
    }

    for (String target : distribute) {
      attributes = new HashMap<>();
      attributes.put("key", target);
      Neo4j.createNode(null, attributes);
      Neo4j.addRelation(getKey(), target, "as", null);
    }
  }
